...
app = FastAPI(
    ...
    root_path="/client",  # Handle /client prefix from ALB
)
...
def custom_openapi(request: Request = None):
    ...
    if request:
        ...
        # If accessed through API Gateway (/api/v1/client), use that path
        if '/api/v1/client' in path:
            # Extract the base path (everything before /openapi.json or /docs)
            base_path = '/api/v1/client'
