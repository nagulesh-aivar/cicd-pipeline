...
class TransactionLogMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        await self.store_to_mongo(log_entry)
        await self.write_log_locally(log_entry)
        return response
    
    async def store_to_mongo(self, log_entry: dict):
        """Store each log JSON into MongoDB."""
        try:
            db = await get_db()
            collection = db["transactions_logs"]

            mongo_doc = TransactionLogModel(
                user="anonymous",
                ip=log_entry["ip"],
            ...
            await collection.insert_one(mongo_doc.model_dump())
            logger.info("Stored transaction log in MongoDB")

        except Exception as e:
            logger.error(f"Failed to store log in MongoDB: {e}", exc_info=True)
